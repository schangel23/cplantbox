#!/usr/bin/env python3
"""S7 D.0 baseline capture: maize_calibrated FA-on, 180 days.

Day-180 companion to `capture_d0_faon_maize.py` (130d). Same harness shape,
same FA-on bake-time helpers (kept post-S3 as bake-time tools); only
SIM_DAYS / OUT_PATH / slug differ. Captures `mainstem_top_z`,
`topmost_leaf_insertion_z`, and SHA against the post-S2.D + S3 baked
maize_calibrated.xml. Used by the ADR §S7 endpoint regression gate
(≤ ±3 cm day-180 mainstem top, ≤ ±2 cm topmost leaf z vs baseline).

Usage (from /home/lukas/PHD/CPlantBox):
    cpbenv/bin/python3 dart/coupling/tests/baselines/capture_d0_faon_maize_180d.py
    cpbenv/bin/python3 dart/coupling/tests/baselines/capture_d0_faon_maize_180d.py --verify
"""
...
OUT_PATH = BASELINE_DIR / "d0_maize_calibrated_faon_s3b_180d.json"
...
def capture_signature(plant) -> dict:
    ...
    return {
        "slug": "maize_calibrated_faon_s3b_180d",
...
def main():
    ...
    parser.add_argument("--verify", action="store_true",
                        help="re-run and compare against stored 180d FA-on baseline")
